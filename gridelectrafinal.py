...
wandb.login(key="1625203c7b0ca77ae80196cde69a8f2de375471f")
...
sweep_id = wandb.sweep(sweep_config, project="ELECTRA")
...
#TRANSFORMER MODEL
pretrained_model =  ElectraForSequenceClassification.from_pretrained("bhadresh-savani/electra-base-emotion", return_dict = True).base_model

class CustomModel(nn.Module):
    def __init__(self, pretrained_model, num_labels):
        super(CustomModel, self).__init__()
        self.pretrained_model = pretrained_model
        self.fc_layer = nn.Linear(pretrained_model.config.hidden_size, num_labels)

    def forward(self, input_ids, attention_mask):
        # Pass input through the pretrained model
        outputs = self.pretrained_model(input_ids=input_ids, attention_mask=attention_mask)
        logits = self.fc_layer(outputs['last_hidden_state'][:, 0, :]) # Using the [CLS] token's representation in last hidden state
        return logits
...
def train():
    ...
    model = CustomModel(pretrained_model, num_labels=2)
    model.to(device)
    params_to_include = ['fc_layer.weight','fc_layer.bias']
    ...
    for epoch_i in range(0, epochs):
        ...
        for batch in validation_dataloader:
            ...
            with torch.no_grad():        
                val_logits = model(b_input_ids,attention_mask=b_input_mask)
                criterion = nn.CrossEntropyLoss()
                loss = criterion(val_logits,b_labels)
